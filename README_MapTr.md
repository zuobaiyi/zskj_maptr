...
## Data & Calibration Requirements (Camera / Multi-view)

MapTR (and related BEV-based view transformation modules) relies on accurate camera geometry to align multi-view features into a unified BEV space. If your camera intrinsics/extrinsics or image pre-processing are inconsistent, the BEV alignment may become blurry, show ghosting/double-lines, or collapse in complex scenes.

### 1) How to quantify “accurate” calibration (actionable targets)

#### A. Intrinsics (pixel-domain)

After calibrating with a checkerboard / AprilTag, evaluate the **reprojection error**:

- **Note**: reprojection error is reported in **pixels**, so it depends on the image resolution (e.g., 1600×900 vs resized inputs), lens FOV, and corner/tag detector quality.
- Typical guidance (treat as rules-of-thumb, not strict pass/fail):
  - **Excellent**: mean reprojection error **≤ 0.5–1.0 px**
  - **Usually usable** (outdoor / wide-FOV / lower-quality targets): mean reprojection error **~ 1–2 px**
  - **Risk**: **> 2 px** may start to degrade BEV alignment (lane/curb edges may jitter or show ghosting), especially at long range or near image borders.

If you apply **undistortion**, make sure to use the **post-undistortion intrinsics** (effective focal length / principal point may change). Otherwise the projection becomes inconsistent even if the raw calibration report looks good.

#### B. Extrinsics (BEV-domain alignment)

Extrinsic errors directly translate to BEV misalignment. For typical BEV grid/voxel scales (often ~0.15–0.5 m depending on configs):

- Tiered guidance (rules-of-thumb):
  - **Excellent**: translation **≤ 2–5 cm**, rotation **≤ 0.2–0.5°**
  - **Often acceptable**: translation **≤ 5–10 cm**, rotation **≤ 0.5–1.0°**
  - **Risk**: translation **> 10 cm** or rotation **> 1°** can noticeably misalign long-range structures and create broken/double-lane artifacts in BEV.

If rotation error approaches **~1°**, long-range projection error is amplified and BEV may show broken or double-lane structures.

### 2) Most practical acceptance tests (recommended)

#### Test 1: LiDAR-to-image overlay (strongly recommended)

For a synchronized frame (LiDAR + 6 images), project LiDAR points onto each camera image using your projection (e.g., `lidar2img`). Check alignment on rigid edges:

- curbs, walls, poles, parked cars, building outlines
- Near range (10–20 m) should be tightly aligned; far range can be slightly looser but should not have systematic bias.

Common failure patterns:

- All cameras shift in the same direction: likely intrinsics/undistortion mismatch or resize/crop not reflected in projection.
- Only one camera is off: that camera’s intrinsics/extrinsics is wrong.
- Near range OK but far range drifts: often focal length / distortion modeling is off, or there is a small extrinsic rotation error.

#### Test 2: BEV output “ghosting” check

In static scenes (straight road with clear lane markings), if the BEV output shows **double lines / ghosting / cross-over**, it is usually a sign that camera calibration or image augmentation/projection matrices are not consistent.

### 3) Keep projection consistent with image pre-processing

If your input images are resized/cropped/padded/rotated, you must keep the projection consistent with the post-processed pixel coordinates. In this repo:

- Image scaling can update `lidar2img` and `img_aug_matrix` (see `RandomScaleImageMultiViewImage`).
- More complex transforms (crop/rotate/non-uniform resize) should also be reflected in the corresponding image augmentation matrix, otherwise BEV alignment will degrade.
