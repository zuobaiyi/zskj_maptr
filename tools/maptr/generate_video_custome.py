...
def main():
    ...
    file_list = sorted(os.listdir(args.visdir))
    ...
    for file in file_list:
        ...
        if os.path.isdir(file_path):
            ...
            if map_img is None:
                raise FileNotFoundError(f"Prediction map image not found: {osp.join(file_path,PRED_MAP_NAME)}")
            
            # If GT map image doesn't exist (inference-only mode), create a black placeholder
            if gt_map_img is None:
                # Create a black image with same size as pred map
                gt_map_img = np.zeros_like(map_img)
